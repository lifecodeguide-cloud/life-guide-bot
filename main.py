...
purpose_outro_keyboard = InlineKeyboardMarkup(
    inline_keyboard=[
        [InlineKeyboardButton(text="Самое интересное дальше 👇", callback_data="show_next_block")]
    ]
)
...
NEXT_BLOCK_TEXT = (
    ...
    "Сейчас сюда можно вставить следующий блок."
)
...
def has_date_data(data: dict) -> bool:
    return bool(
        data.get("date")
...
# =========================
# ОПЛАТА / ПЛАТНЫЙ ПОТОК
# =========================
async def open_paid_flow(message: Message, user_id: int):
    data = get_user(user_id)

    if not has_date_data(data):
        update_user(user_id, paid=True, stage="awaiting_date_after_payment")
    ...
    update_user(user_id, paid=True, stage="purpose_intro_shown")
...
async def send_purpose_number(callback: CallbackQuery):
    ...
    if not has_date_data(data):
        update_user(user_id, stage="awaiting_date_after_payment")
        await callback.message.answer(
            "Не вижу дату рождения.\n"
            "Отправьте её в формате ДД.ММ.ГГГГ"
        )
        return
    ...
    if not purpose_text:
        await callback.message.answer(f"Не найден текст для предназначения {purpose}.")
        return

    update_user(user_id, stage="purpose_number_shown")
...
async def start_handler(message: Message):
    ...
    if text.startswith("/start paid"):
        ...
        if paid_user_id != user_id:
            await message.answer(
                "Оплата подтверждена ✅\n\n"
                "Но продолжение открыто не тем Telegram-пользователем.\n"
                "Откройте его из того Telegram, где вводилась дата рождения."
            )
            return

        await open_paid_flow(message, user_id)
        return
    ...
    if data.get("paid"):
        ...
        update_user(user_id, stage="purpose_intro_shown")
        return
...
async def date_handler(message: Message):
    ...
    update_user(user_id, stage="soul_intro_shown")
...
async def show_soul_handler(callback: CallbackQuery):
    ...
    soul = data.get("soul")
    soul_text = SOUL_TEXTS.get(soul)

    if not soul_text:
        await callback.message.answer(f"Не найден текст для числа души {soul}.")
        return
    ...
    update_user(user_id, stage="soul_shown")


# =========================
# ВСТУПЛЕНИЕ К ЭКСПРЕССИИ
# =========================
...
async def show_expression_intro_handler(callback: CallbackQuery):
    ...
    update_user(user_id, stage="expression_intro_shown")
...
async def show_expression_handler(callback: CallbackQuery):
    ...
    expression_text = EXPRESSION_TEXTS.get(expression)

    if not expression_text:
        await callback.message.answer(f"Не найден текст для экспрессии {expression}.")
        return
    ...
    update_user(user_id, stage="expression_shown")
...
async def open_sales_handler(callback: CallbackQuery):
    ...
    update_user(user_id, stage="sales_shown")


# =========================
# ПРЕДНАЗНАЧЕНИЕ — ЧИСЛО
# =========================
@dp.callback_query(lambda c: c.data == "show_purpose_number")
async def show_purpose_number_handler(callback: CallbackQuery):
    await safe_answer_callback(callback)
    await send_purpose_number(callback)


# =========================
# ПРЕДНАЗНАЧЕНИЕ — ПОСЛЕСЛОВИЕ
# =========================
...
async def show_purpose_outro_handler(callback: CallbackQuery):
    ...
    update_user(user_id, stage="purpose_outro_shown")
...
async def show_next_block_handler(callback: CallbackQuery):
    ...
    update_user(user_id, stage="next_block_shown")
